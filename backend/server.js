...
// Add a package (Agent)
app.post("/api/packages", upload.array('images',10), async (req,res)=>{
...
inclusions,exclusions,itinerary,status,agent_id,images,package_types)
...
const hotelsResult = await db.query(
    "SELECT day_number, hotel_name FROM hotels WHERE package_id = $1 ORDER BY day_number",
    [req.params.id]
);
pkg.hotels = hotelsResult.rows;
...
app.post("/api/admin/packages/:id/add-image", upload.array('images',10), async (req,res)=>{
...
JSON.stringify(data.package_types || []),
r.package_id
]
);
...
app.post('/api/agent/update-package', upload.array('images', 10), async (req, res) => {
    try {
        const {
            package_id, agent_id,
            title, destination,
...
app.get('/admin/package-requests', async (req, res) => {
    try {
        const result = await db.query(
            `SELECT pr.*, u.full_name as agent_name, p.title as current_title
             FROM package_requests pr
    ...
    try {
        ...
        await db.query(
            `UPDATE packages SET
                ...
                package_types = $12, images = COALESCE($13, images)
             WHERE id = $14`,
            [
