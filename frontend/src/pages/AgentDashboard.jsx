...
const renderBookings = () => {
  const filteredBookings = bookingPackageFilter
    ? bookings.filter(b => Number(b.package_id) === Number(bookingPackageFilter))
    : bookings;
  ...
  return (
    <>
      ...
      <div style={{ display: 'flex', alignItems: 'center', marginBottom: '16px', gap: '10px' }}>
        <label htmlFor="package-filter" style={{ whiteSpace: 'nowrap' }}>Filter by Package:</label>
        <select
          id="package-filter"
